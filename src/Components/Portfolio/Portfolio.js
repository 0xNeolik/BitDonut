...
function Portfolio(props) {
  ...
  const handleCoin = (e) => {
    ...
    let thisCoin = props.coins.filter((coin) => {
  ...
  let getInfoOfPortfolio = (active) => {
    if (active === "total") {
      let totalOfCoins = portfolio?.map((oneInPortfolio) => {
        let matches = props.coins.filter((oneInAllCoins) => {
      ...
      <div id="Display">
        <div>
          ...
          <div id="display-cards">
            {portfolio.map((onecurrent) => {
              let currentCoin = props.coins.filter((coin) => {
                return coin.symbol === onecurrent.symbol;
              });
              return (
                <CoinPortfolioCard
                  onecurrent={onecurrent}
